...
//     const response = await get_All(`${PRODUCTS_URL}?${params.toString()}`);
//     return response.data;
//   } catch (error) {
//     console.error(
...
//   try {
//     const response = await get_All(`${PRODUCTS_URL}/${id}`);
...
//     const response = await get_All(
//       `${PRODUCTS_URL}/search?q=${encodeURIComponent(
//         query,
//       )}${categoryLvl4Query}`,
//     );
//     return response.data;
...
// Fetch product details by category and product name
export async function getProductDetails(categoryId, productName) {
...
export async function getAllCategories() {
  const res = await fetch(CATEGORIES_URL, { next: { revalidate: 60 * 5 } });
...
export async function getPromoProducts() {
  const res = await fetch(`${PRODUCTS_URL}/promo`, {
    next: { revalidate: 60 * 5 },
  });
...
export async function getProductsBycategoriesLvl2(
  categoryName,
  categoryLvl4Id,
) {
  const categoryLvl4Query = categoryLvl4Id
    ...
    : "";
  const res = await fetch(
    `${API_URL}/category/${encodeURIComponent(categoryName)}${categoryLvl4Query}`,
    { next: { revalidate: 60 * 5 } },
  );
...
export async function searchProducts(query, categoryLvl4Key) {
  let categoryLvl4Query = "";
  if (categoryLvl4Key) {
    const isObjectId = /^[a-f\d]{24}$/i.test(String(categoryLvl4Key));
  ...
  const res = await fetch(
    `${PRODUCTS_URL}/search?q=${encodeURIComponent(query)}${categoryLvl4Query}`,
    { next: { revalidate: 60 * 5 } },
  );
...
export async function getHomeCategories() {
  const res = await fetch(`${PRODUCTS_URL}/home-categories`, {
    cache: "no-store",
  });
...
export async function getTrendingProducts() {
  const res = await fetch(`${ARTICLES_URL}/trending`, {
    next: { revalidate: 60 * 5 },
  });
